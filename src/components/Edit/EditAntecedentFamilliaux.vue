    ...
    <div class="absolute w-3/5 bg-white rounded-md shadow-xl z-50">
      <div class="w-full px-5 pt-5 pb-2 flex justify-between items-center">
        <div class="flex items-center space-x-3 w-11/12">
          <h2 class="text-lg text-blue-600 font-bold uppercase">Modifier l'antecedent famillial</h2>
        </div>
        <div class="">
...
const props = defineProps({
  antecedentfamilliaux: Object
})

const emit = defineEmits(['close'])

const form = reactive({
  nom: props.antecedentfamilliaux.nom,
  satuts: props.antecedentfamilliaux.satuts,
...
const submit = () => {
  axios.put(`antecedent_familliauxs/${props.antecedentfamilliaux.id}`, form)
    .then((res) => {
      emit('close')
    })
    .catch((err) => {
