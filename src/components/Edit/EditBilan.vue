    ...
    <div class="absolute w-3/5 bg-white rounded-md shadow-xl z-50">
      <div class="w-full px-5 pt-5 pb-2 flex justify-between items-center">
        <div class="flex items-center space-x-3 w-11/12">
          <h2 class="text-lg text-blue-600 font-bold uppercase">Modifier le bilan</h2>
        </div>
        <div class="">
          ...
          <form action="" method="post" class="">
            <div class="grid grid-cols-2 gap-8 py-5 mb-10">
              <div class="flex flex-col space-y-1">
                ...
                <input type="text" name="type" id="type" class="" v-model="form.type" placeholder="Entrez le diagnostic"/>
              </div>
              <div class="flex flex-col space-y-1">
                <label for="resultats">Resultats</label>
                <input type="text" id="resultats" v-model="form.resultats" placeholder="Entrez l'examen"/>
              </div>
              ...
              <div class="flex flex-col space-y-1">
                ...
                <textarea type="text" v-model="form.description" id="description" placeholder="Saisissez l'hypothese"></textarea>
              </div>
            </div>
...
const form = reactive({
  type: props.bilan.type,
  resultats: props.bilan.resultats,
  date: props.bilan.date,
  description: props.bilan.description,
...
const submit = () => {
  axios.put(`bilans/${props.bilan.id}`, form)
    .then((res) => {
      emit('close')
    })
    .catch((err) => {
